 ...
 */
package mcp.wdl.handler.entity;
...
public class VillagerHandler extends EntityHandler<EntityVillager, ContainerMerchant> {
	...
	@Override
	public IChatComponent copyData(ContainerMerchant container, EntityVillager villager, boolean riding) throws HandlerException {
		...
		if (CAREER_ID_FIELD != null) {
			IChatComponent displayName = merchant.getDisplayName();
			if (!(displayName instanceof ChatComponentTranslation)) {
			...
			ChatComponentTranslation displayNameTranslation = ((ChatComponentTranslation) displayName);
			...
			return new ChatComponentTranslation("wdl.messages.onGuiClosedInfo.savedEntity.villager.tradesAndCareer", displayName, profession, career);
		} else {
			return new ChatComponentTranslation("wdl.messages.onGuiClosedInfo.savedEntity.villager.tradesOnly");
		}
	}
