...
import mcp.wdl.EntityRealigner;
import mcp.wdl.HologramHandler;
import mcp.wdl.MessageTypeCategory;
import mcp.wdl.VersionConstants;
import mcp.wdl.WDL;
import mcp.wdl.WDLMessages;
import mcp.wdl.config.settings.MiscSettings.ExtensionEnabledSetting;
...
public class APIImpl implements WDLApi.APIInstance {
	...
	@Override
	public <T extends IWDLMod> List<WDLApi.ModInfo<T>> getImplementingExtensions(
			Class<T> clazz) {
		if (clazz == null) {
			throw new IllegalArgumentException("clazz must not be null!");
		}
		List<WDLApi.ModInfo<T>> returned = new ArrayList<>();

		for (WDLApi.ModInfo<?> info : wdlMods.values()) {
			if (!info.isEnabled()) {
				continue;
			}
			...
			if (clazz.isAssignableFrom(info.mod.getClass())) {
				...
				WDLApi.ModInfo<T> infoCasted = (WDLApi.ModInfo<T>)info;
	...
	@Override
	public <T extends IWDLMod> List<WDLApi.ModInfo<T>> getAllImplementingExtensions(
			Class<T> clazz) {
		if (clazz == null) {
			throw new IllegalArgumentException("clazz must not be null!");
		}
	...
	private static class ModInfoImpl<T extends IWDLMod> extends WDLApi.ModInfo<T> {
