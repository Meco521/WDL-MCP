...
import mcp.wdl.WDL;
import mcp.wdl.WDLPluginChannels;
...
public class GuiWDLEntityRangePresets extends WDLScreen {
	...
	public GuiWDLEntityRangePresets(GuiScreen parent, WDL wdl, IConfiguration config) {
		...
		if (result) {
			...
			if (id == ID_VANILLA) {
				...
				for (String entity : EntityUtils.STANDARD_SPIGOT_MANAGER.getProvidedEntities()) {
					EntityUtils.SpigotEntityType type = EntityUtils.STANDARD_SPIGOT_MANAGER.getSpigotType(entity);
